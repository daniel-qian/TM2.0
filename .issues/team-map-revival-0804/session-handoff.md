...
**#106 三棒全做完了：B1 骨架 / B2 focus 机器 / B3 HUD-lite + 门。#107 B4 收拢态 0819 也做完了。**
...
- ✅ **B4 部门收拢态（#107）0819 做完了**：触发条件按 kickoff 拍板从「首个 40 人真租户」
  换成「首次碰面要演示」，火情判据 = 警报药丸那把尺。四件全落地（布局层 / 渲染层 /
  开局镜头对火情 / 项目列同步），逐条账见 [`receipt-107-b4.md`](receipt-107-b4.md)。
  门 80 条 · 纯函数 26 条 · 变异 9 发 · 像素 8 张（存量 66 张零漂移，现共 74 张）。
  ⚠ 一处对票面的偏离已知会：**收拢卡不印人数**（撞 B3 的判据 F1 与 ADR-0023），要推翻改个常量。
