...
**8/8 绿，50 张基线 md5 逐字节一致**（比对前后各存一次 md5 全表 diff ＝ 真比对不是首写）。

🔴 **但这个「零漂移」是判据够不着，不是「改了没影响」**——必须这么读：
`visual.spec.mjs` 采的 room 那 4 张是 **contextId===null 的无材料态**（「眼下还没有可推理的
依据」那张卡），而本票改的全部在 **contextId 非空**那一侧（docked composer / 开场块 /
会话流 / 停止 / 附件）。像素门对本票的改动面**接近零覆盖**。
→ 视觉证据是下面那 26 张手拍截图，不是这 8/8。

⚠ **worktree 里的像素结果一律不算数**，本轮又实证一轮而且差点被骗：电池第一次跑 B 区时
visual 因为写死 `localhost:5173` 而 CONNECTION_REFUSED、**一张都没比对**，却在目录里留下了
50 张 mtime 是当时的 PNG。「基线文件在」≠「比对过了」——**先翻日志**。
清掉重来后是首写 → 复跑 8/8 绿，**那只证稳定**。
