...
- 合 main 后在主检出复跑 visual：**4/4 绿（36 张，零漂移）**——「静息态 DOM 与改造前逐字节
  等价」的承诺兑现，无需重冻。比对是真的：基线 = #63 刚重冻的 36 张、preview 确认 5173 =
  主检出 dist、后端在场（demo 门卡在——后端够不着时 home 会先红，绿即证明探测通了）。
- 像素基线只锚静息态，**交互态另截图人眼过**（`t64-shots/` 六张：desktop/mobile ×
  room 弹层 / room chip / 胶囊弹层）：五筛选 chip、两位林小满各带部门+职位+类目角标、
  chip 药丸带 × 移除键、移动端弹层 wrap——零折行溢出、零布局破碎，纸感皮统一。
