...
| 像素基线 | 改了界面 → 归主检出重冻（见文末「留给合 main 之后」） |
...
**像素基线要在主检出重冻并人眼过**。理由不是流程洁癖：`__snapshots__/` 是 gitignore 的单机产物、
**每个 worktree 一份**，在这个 worktree 里重冻等于没重冻。本票改了资料库屏（多一段
「给这家公司补资料」）与详情浮层（多一个出处角标），`files-*` 那几张会动。

⚠ 另一条：`visual.spec.mjs` 跑在 `?transport=stub` 下且从不上传，所以 `contextId` 恒为 null——
**补资料那一段在基线里整段不渲染**（它三条否决的第一条就是没有 contextId）。会动的是第②段那两句
改口的文案（`againTitle`/`againBody`），以及标题从「上传新一批」改成「另建一份画像」。
