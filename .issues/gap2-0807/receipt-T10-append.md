...
| 后端离线全套 | **3927 passed / 115 deselected / 4 xfailed / 0 failed**（本票新增 **45** 条；T10 自己那一刻是 3843→3893，这里是**合完 T9 之后重跑**的终值） |
| `@needs_db` 全套（真 PG） | **106 passed**（HITL 基线 98 + T9 的 4 + 本票 **4**，7m27s） |
| 前端电池 A 区 | **28/28 绿**（含本票新门 `append-story` 18 判据；合 T9 前是 27/27） |
| 前端电池 B 区 | `data-boundary` 37/37 绿 |
| 前端电池 C 区 | 3/3 绿 |
| i18n | en 叶子键 **976**，**孤儿 0**（本票 9 条）；zh/en 键集逐键对齐 |
