...
> ### 🔴 07-19 更正 · 上面这个代码块的前两行已经执行掉了（原文保留，用于追溯）
>
> 本节写于 AFK 开跑之前。**授权范围 ③ 的 `git push` 已经用掉了**，所以基线变成：
>
> ```
> origin/main   de47ffe   ← 线上前端现在是这一版构建的（07-19 推了 23 个提交）
> 本地 main     de47ffe   ahead 0，工作区干净
> 线上后端      avery-agent:zh512（源 512b11d）—— 没变，见下
> ```
>
> * **拍板 ① 已落地**：裸链默认翻成 v02（`7ad968b`），`?v=1` 保留 v01 逃生门。
>   实现是翻 `resolveVersion()` 的字面量，**刻意没加构建期 env**——`mode.ts` 已经有一套
>   「本地默认 / 生产 env」的分裂，再加第二套会让裸 URL 的行为在门里和生产上分叉，
>   而两边门全绿。一个只在生产上错的默认值查不出来。
> * **拍板 ② 停在边界上，后端一行没动。** 复核发现问题不在 `0008`，而在 `6f838f3` 自己：
>   它的编码梯子只有 `("gb18030","big5")`，`decode_text` 取第一个不抛异常的那级，而
>   **gb18030 解 Shift_JIS / EUC-KR 不抛异常**——日文/韩文花名册会被静默读成一串编造的
>   汉字并标记「已读入」，且无任何存疑信号。**那是编造数据，比它要修的「吃掉数据」更坏。**
>   修它的 `a45bb4a` 在授权的两个提交之外，所以没扩大范围，等 Danny 拍。
> * 顺带查出**比 ② 更宽的风险**：`eval-harness/requirements-service.txt` 一个版本都没钉
>   （全 `>=`），`Dockerfile` 的 `FROM python:3.11-slim` 也没钉 digest。
>   **任何一次后端重建都同时是一次范围不受控的依赖升级**，不只是上你挑的那几个补丁。
> * §4 的门基线随之变成 **41 / 15 / 4 / 4**（新增 `verify-bare-url-shell.mjs` 4 条断言）。
>   §4 里预告的 `verify-zh-purity.mjs` 口径反转**已经改了**（`995e8eb`）。
> * ⚠️ **§4 那句「中文纯度 v01 0 处」是个假数**：那道门只采样 v01 五屏里的两屏，
>   从不访问 v01 自己的「往哪走」屏，而那屏带着与 v02 同源的**同样 9 条**混排字符串。
>   v01 的真实数字是 **9，不是 0**。文案不动（issue #19 是 Danny 的品牌判断），但别再拿
>   「v01 0 处」当验收条件——它只证明了门没看那一屏。
> * ⚠️ **§6 假设「所有 worktree 都有 node_modules junction」，本 worktree 根本没有。**
>   而且共享的 `D:\avery\node_modules` 本身就是残缺的：整个 `@babel` scope 和 `.bin/` 都不在，
>   所以 `npm run typecheck|lint|build` 直接找不到命令、vite dev server 起不来。
>   **先于本次会话存在，不是本次造成的，也没修**（修 = `npm install`，§6 明令禁止）。
