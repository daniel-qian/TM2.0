...
## 镜像与容器状态（最新）
```
当前镜像 avery-agent:zh512sub2-20260720-102149  ← 含 parse-crash 兜底，生产在跑，healthy
上一版   avery-agent:zh512sub-20260720-095401  (49c126c1b38d) 未删
最新回滚 avery-prev-20260720-102149  → 上一版(zh512sub)容器，Exited 完整保留
更早回滚 avery-prev-20260720-095401  → 最初 zh512 容器，Exited 完整保留
旧镜像   avery-agent:zh512 (df09186d6a80) 未删
```
构建源：`origin/deploy/zh512-subset-0720`（HEAD `e5a8743`）。服务器 `/home/admin/build-zh` detached 在 `e5a8743`。

## 一键回滚（回到上一版 zh512sub）
```bash
ssh admin@8.211.28.11
sudo docker rm -f avery
sudo docker rename avery-prev-20260720-102149 avery   # 回到 zh512sub（无 parse 修复）
sudo docker start avery
curl -s http://127.0.0.1:8137/health   # 确认 {"status":"ok"}
# 要一路回到最初 zh512，用 avery-prev-20260720-095401
```
