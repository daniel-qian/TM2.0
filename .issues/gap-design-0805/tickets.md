...
## T8 · e2e-loop：AFK 全链端到端循环自检（issue #57，收官自检票）

**前置**：T1–T7 全合 main（`403910a`）。**全程本地栈、零真 LLM、不部署生产、不碰生产数据。**
跑完出 go/no-go → 统一上产 → Danny 的 HITL 轮。完整规格见 issue #57（正源）。

一句话：把两句对客承诺当**一个整体**考——本地真栈反复走「传材料→建模板→铸链→员工填（含模仿攻击/
过期/重复）→资料同构断言→向量只嵌增量→议事室引用→人卡回流→今天页三规则→双语断言→截图」，
自跑自验自修，**连续两轮零新发现**才收官；收官门=后端离线+needs_db+前端 A→B→C+像素+i18n-orphans 全绿。
纪律：真浏览器打真本地后端、fixture 不得自考自答、语义歧义只记录留给 HITL；
像素基线不密闭（先起对带 demo seed 的后端）；A 区对新部件可能零覆盖（扩 t3-story.mjs 别自欺）。
产出：`receipt-T8-e2e.md`（逐轮清单+go/no-go）+ 截图包 + 重写 progress.md + 关 issue。
