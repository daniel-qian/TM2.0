...
> ✅ **这一格空了两周，现在由一道门顶上了（2026-08-05 · ADR-0034 拍板 6）。**
> F 段要人手走的那条链——注册→登录→认领→双账号隔离——已经被
> `.issues/onboarding-accounts-0805/verify-account-e2e.py` 自动跑通：真 Supabase auth、
> 真 access_token、真后端验证、真绑定行、两个真账号之间的同体 404。连跑两遍全绿（第二遍证
> 清理幂等），并做过 born-red 自证（把隔离判据反写成"期望 200"，门确实变红）。
> 跑完生产 `auth.users` 回到 0 行，零留尸。
> **人手走查被自动化门取代的口径变化留痕在此**：F 段不再是唯一证据来源；凭据墙的现行条文
> 见 `roles.md` 的「凭据墙」一节（测试前缀口子 + 真实账号仍人手）。
