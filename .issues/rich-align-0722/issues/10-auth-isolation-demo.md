...
- **凭据墙**：登录输入账号密码是 Danny 人手环节，agent 不代填任何凭据；本片交付的是「可演可验的线」——机器探针只打到 401/404/探测边界与（可 mock 账号解析层的）隔离断言，真登录全流程归 HITL。
  > 🔴 **口径已修订（2026-08-05 · ADR-0034 拍板 6，条文在 `roles.md` 的「凭据墙」一节）。**
  > 上面这句写于 07-22，当时的绝对句是「真登录全流程归 HITL」。事后坐实它**走不到**：
  > 到 08-05 为止生产 Supabase 的 `auth.users` 是 **0 个用户**——一年里没有任何人或门走过这条链。
  > 现行口径：**约定前缀的一次性测试邮箱（`avery-e2e+<时间戳>@…`）agent 可自动注册/登录/清理；
  > 除此之外的任何账号仍然人手，agent 绝不代填。** 真跑的门是
  > `.issues/onboarding-accounts-0805/verify-account-e2e.py`。
  > 引用本条时请连这段一起引——别把上面那句绝对话单独抄走。
