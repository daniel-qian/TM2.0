// rich-align-0722 · issue 01 · e2e 探针 —— 富字段（进度/风险）走真管道 + absent≠none。
//
...
const WITH = ['# 婚宴项目', '负责人：小王', '状态：进行中', '进度：58%', '风险：高/雨季无备选场地', ''].join('\n')
...
async function uploadAndRead(text) {
  ...
  let overlay = { rings: 0, riskRows: 0 }
  if (payload) {
...
rec('WITH · projects 屏渲染高风险徽章 + 进度条',
  w.dom.riskHigh === 1 && w.dom.progressFills === 1, JSON.stringify(w.dom))
rec('WITH · 详情浮层有环形进度 + 风险行', w.overlay.rings === 1 && w.overlay.riskRows === 1,
  JSON.stringify(w.overlay))
...
rec('WITHOUT · projects 屏不画风险徽章', n.dom.riskBadges === 0, `徽章 ${n.dom.riskBadges} 个`)
...
rec('WITHOUT · 详情浮层无环形无风险行', n.overlay.rings === 0 && n.overlay.riskRows === 0,
