...
## 六、回答部署线：前端在旧后端下会不会崩（2026-07-19 凌晨实测）

部署线在回执 §84-99 提了一件我们广播里没说的事，并向我们提了一个问题：
...
**线上后端跑的是本波之前的镜像。** 所以本波全部后端产出——feat-053 的 auth 端点、
feat-056 给 `/team/{id}` 加的 `decisions[]`、feat-054 的粒度门、feat-060 的红线补漏、
`/advise` 错误路径的 NameError 修复，以及第二轮对抗审查正在修的四条后端 blocker——
**目前都不在线上**。重建后端需要 Danny 单独拍板（会连带把迁移 `0008` 推上生产库）。

他们问：**前端在旧后端下是否优雅降级？特别是 `decisions[]` 缺失时有没有 `.map` of undefined。**

### 实测方法

写了一个只讲 2026-07-17 契约的假后端（`/team` 回包**没有 `decisions` 键**、`/account/*` 全 404、
其余按老契约），把前端指过去，逐屏走一遍。

### 结论：**降级干净，可以放心把当前前端留在线上**

| 项 | 结果 |
|---|---|
| ingest → 团队数据 | 正常，2 个项目载入 |
| `decisions` 键缺席 | `rawTeam.decisions === null`，**没有 `.map` of undefined，不崩** |
| 9 个屏逐个渲染 | 全部有内容，**0 个 pageerror** |
| 账号入口（`/account/*` 404） | `.lite-auth-toggle` **不渲染**——没有假装能登录，符合诚实标注要求 |
| console 错误 | 1 条，且是 React unique-key **警告**，非崩溃（见下） |

### 附带修掉的一条

`HomeScreen` 差距摘要列表用裸 `gap.id` 作 key。跨文档重复的项目会产出同 id 的 gap
（后端 issue #10，去重在 LLM 抽取路径上失效），会撞键。本屏另外两个列表已经带下标了，
这是漏的第三处。🔴 **没有在前端去重**——那会把后端的 bug 藏起来。
