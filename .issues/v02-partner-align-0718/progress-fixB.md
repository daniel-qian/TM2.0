...
> ## 🛑 07-19 更正 · 读 M2 之前必看：那两个「真值」不是生产真值
>
> **本文 §M2 说上传上限的真值是「15 个文件 / 8 MiB」。那是代码默认值，不是线上跑的数字。**
> 生产环境显式覆盖了它（已上机核过 `sudo docker exec avery env | grep AVERY_MAX`）：
>
> ```
...
> **所以生产上的真值就是 10 个 / 10 MiB，现有中文文案「10 个文件、每个 10MB」是对的。**
> 照 §M2 去把它改成 15 / 8 MiB，等于**把对的改成错的**。
>
> 部署线在 07-19 已经差点这么改了，是上生产核了一遍 env 才拦住。这条陷阱写进了
> `.issues/v02-joint-0719/kickoff-afk.md` §5①，但**当时没回到本文源头撤回**——
> 于是下一个读到 §M2 的人还是会重新推出那个错结论。现在补上。
>
> **§M2 成立的那半仍然成立**：前端硬编码一份会和服务端 env 漂移的副本，本身就是缺陷。
> 正解就是本文写的那条——**删掉副本、转达服务端 413 body**，而不是把副本里的数字换一批。
> 下面 §M2 原文一字未删，只是别再拿它的括号里那两个数当真值。
...
> 🔴 **07-19 更正（详见本文顶部横幅）**：括号里那两个数是**代码默认值**。
> 线上 env 显式覆盖成 `AVERY_MAX_FILES=10` / `AVERY_MAX_UPLOAD_BYTES=10485760`，
> **生产真值 = 10 个 / 10 MiB**，所以原文案那两个数字在生产上是**对的**，
> 「用户压到 10MB 重试永远撞墙」在生产上也**不会发生**。
> 别照这段去改文案。这段成立的部分是「前端不该硬编码第三份副本」——
> 而下面给的修法（删副本、转达服务端 413）本来就与真值是多少无关，仍然正确。
