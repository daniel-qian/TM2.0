...
> ### ✅ 07-19 更正 · 这条最凶的脆弱点被拍板 ① 反转掉了（原文保留）
>
> 加粗那句描述的失败模式**不再存在**。裸链默认已从 v01 翻成 v02（`7ad968b`，
> `resolveVersion()` 缺省返回 `'2'`），所以**丢掉 `v=` 现在落回的是 v02**，不是 v01。
> 换句话说：这条路由工作里最怕的「粘性 query 漏一处 = 客户被弹回冻结旧壳」，
> 现在的最坏后果只是「参数回到默认」，而默认就是对的那一版。
>
> 下面那套「默认全带走 + 极小黑名单」的口径**仍然要守**——`mode` / `lang` / `look` 丢了
> 照样会掉回各自的默认值，且 `?v=1` 逃生门本身也依赖 query 能被带住。
> 变的只是**丢参数的代价从「掉回 v01」降成「回到默认」**。
>
> ⚠️ 另注：本段写的 `lite2/skin.ts` 与 `?skin=` 已被 feat-068 改名为
> `lite2/look.ts` / `?look=`，且**旧的 `?skin=` 不再被识别**（会响亮地 `console.error`，
> 但页面仍按默认 `paper` 渲染）。
