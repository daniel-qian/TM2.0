...
# 复核回合 2 — 复核判 needs-fix，本节记修了什么、没修什么

复核者判定 `verdict: needs-fix`，2 blocker + 1 major + 2 minor。诚实度复核逐条对上，
**唯一一处夸大是我自己写的「0 漏报 0 误报」被写成了无条件结论** —— 实际只在我自己那份语料内成立。
这个批评成立，我接受：语料结构上缺一类形状，补上后 72/72 全漏，其中 64 条是改前会拦、改后放行的**净回归**。

## 病根（三条 finding 是同一个病）

`_NEG_GOVERNS_TIGHT`（round 1）把**裸的评分名词**当成「一个动作被否定了」的证据：
...
但中文里「未/无/非 + 评分名词」几乎总是**名词短语中心语**，不是被否定的动作：

| 措辞 | 真实语义 | round 1 的判读 |
|---|---|---|
| 无绩效数据 | a department with **no perf data**（绩效修饰数据） | 「打分被禁止了」→ 关闸 32 字 |
| 非考核期间 | outside the review period | 同上 |
| 未评级岗位 | an unrated position | 同上 |
| 不能打分 | an intern who **cannot be scored** | 同上 |

代码注释自己写的设计意图是「未/无/非 must govern a SCORING **action** specifically」——
实现却给了一张**名词表**。意图与实现在这里背离，复核者指得准。

## 修了什么

### blocker 1+2 —— `redline.py`：前瞻必须是「动作形状」

新增 `_NEG_GOVERNS_ACTION`，**去掉裸评分名词**，只认真正的动作证据：

```python
_NEG_GOVERNS_ACTION = rf"(?:[{_NEG_COVERB}]|{_NEG_PERSON_OBJ})"
#   coverb（把/对/给/向/让/帮/拿/跟）  或  人称宾语 + 评分词
```

并把原来一张 `_NEG_ZH_IMPERATIVE` **拆成两类**（这是 blocker 2 的关键）：

- `_NEG_ZH_PROHIBITION`（不要/不用/不应/不该/不得/不做/不搞/不给/勿/莫/甭/严禁/禁止/切忌/避免/杜绝/拒绝/谢绝/以免/免得/无需/无须/毋须）
  → 走**宽**类 `_NEG_GOVERNS`。这些是真祈使/及物禁止动词，「杜绝末位淘汰」合法地管一个名词。
- `_NEG_ZH_MODAL`（不准/不许/不能/不可/不必/不宜/不予/不再）
  → 走**动作**类 `_NEG_GOVERNS_ACTION`。这些**会名词化**（「不能打分的实习生」「不再打分之后」），
  跟 未/无/非 是一类，不该跟祈使词放一起。

**误报侧代价 = 0，而且是量出来的不是想出来的**：唯一靠裸名词的三句真否定
（`无打分。` `无排名。` `非评分性质。`）在 `_NEG` **整个关掉**的情况下也不触发任何 violation，
即那三句从来就不是靠这个 cue 保住的（探针：把 `_NEG` 换成永不匹配的正则，50 句里只有 17 句真依赖 cue）。

### major —— 语料的系统性形状缺口

复核者的批评最狠也最对的一条：**变异证明只能证明「规则对我想到的形状有效」，证不了「规则没吞掉我没想到的形状」**。
按他的建议，语料改成**从规则自己的词表笛卡尔积生成**，而不是手写枚举：
...
#  11 个可名词化 cue × 33 个评分名词 = 363 行，redline.py 加词它自己长
```

外加 18 条**手写的真实 HR 措辞**进 `_ORDINARY`（无绩效数据/非考核期间/未评级岗位/不能打分…），
因为生成语料机械但不像人话，两者都要。`_ORDINARY` 66 → 84。

新增 `test_the_nominalised_corpus_is_generated_from_the_rules_own_vocabulary`：
盯住「这份语料必须是派生的」，谁把它冻成字面量列表就跑红 —— 这是对**流程**失败而不只是对 bug 的修。

## 没修什么（skipped，理由在下面，不是偷懒）

### minor：`避免/拒绝/杜绝/无须` + 评分名词仍漏（4 词 × 4 payload = 16）

**这是另一种病，不是同一种。** 这四个是**真禁止**（「避免打分」确实在禁止打分），
任何前瞻收紧都拒绝不了它们 —— 错的不是 cue，是**辖域**：

> 「避免打分的团队里，她排名倒数第一。」 ← 避免打分 管的是「团队」这个定语从句，后面那个分是真的

**修法我已经写出来并量过了，然后主动回退了**：给 `_negated` 加小句/CSV 字段边界
（cue 不许跨 `，。；！？、` 和 ASCII `,;!?`；**冒号不算**，因为「不给她打分：8/10。」是一句话）。
实测**4 词全堵，50 句真否定 0 新误报**。回退的三个理由：

1. **它跟隔壁文件一条明确的契约测试冲突**。
   `test_redline_villa_negation_b3.py::test_negated_window_is_the_scope_of_a_real_cue`
   断言 `「别给她打分,绩效评分2分」` 必须 **PASS** —— 即**要求** cue 能跨逗号；
   它的 docstring 还明写了 "It has no notion of clause or scope"。这是别人**故意钉下**的设计，
   翻它属于对 moat 的设计变更，不该搭本条的便车悄悄过。实测加边界后该文件 2 条测试跑红。
2. **加了边界，本文件的语料会整体变空转**。`_row` 是 `陈思雨,{word},{payload}`，逗号一旦成边界，
   84 词 × 4 payload 全部无条件通过 —— 实测把 cue 换成**裸的**（round-1 之前的病态版本）也只漏 0/66。
   也就是说要在同一次改动里把整份语料重建成无标点行，否则就是**拿一个已知缺口换一个没量过的缺口**。
3. **它会动英文**。本次改动英文侧是逐字节不变的（下面有 1092 串差分证据），加边界就不是了。

所以这条**钉成了一个 characterization 测试**而不是留给下一个人重新发现：
`test_the_clause_scope_gap_is_still_open` —— 它断言**这个 bug 还在**，
docstring 里写清了修法、量到的数、以及为什么回退，并注明「那条线落地时请**删掉**这个测试，不要弱化它」。
`redline.py` 里对应位置也有同样内容的注释块。

**建议单开一线**：小句辖域 + 英文 blast radius + `_row` 语料重建，一起做。

### minor：中文人名不是 person anchor / 繁体「別」误拒

复核者自己判定为 pre-existing、OLD=NEW、不算本条扣分项，并建议各自单开一线。我同意，未动。
但复核者点出的**叠加效应**值得记：「无绩效数据的部门，黄志强排名倒数第一」以前是两层都不拦，
现在第一层（否定词）已经拦住了，anchor 那层仍开。

## 门（回合 2 实跑，全部贴的是真输出）

| 门 | 命令 | 结果 |
|---|---|---|
| 既有测试零回归 | `pytest tests/ -q --ignore=tests/test_redline_holes_060.py` | `906 passed, 61 skipped, 8 xfailed` |
| 全量 | `pytest tests/ -q` | `2815 passed, 61 skipped, 8 xfailed in 18.44s` |
| 本条单跑 | `pytest tests/test_redline_holes_060.py -q` | `1909 passed`（round 1 是 376） |
| **born-red** | 把 `redline.py` stash 回 round 1，再跑本文件 | `1534 failed, 375 passed` |
| 前端 | `npm run typecheck` | 零错 |
| 前端 | `npm run build` | 通过，674.48 kB（chunk 警告是既有的） |

`8 xfailed` 原样 —— **没碰评分闸**（feat-067 的事），H6 那 2 条 strict xfail 没转正。

**born-red 这一行是本回合最要紧的证据**：round 1 的语料对着 round 1 的规则全绿（376 passed），
所以「全绿」本身证明不了任何事。新语料对着 round 1 的规则 1534 条跑红，才说明它真的在量东西。

## 复核者的原始复现，逐条回验

```
=== 复核者的 blocker 端到端用例 ===
  BLOCK  v=1  无绩效数据的部门里，陈思雨排名倒数第一，建议列入末位淘汰名单。   ← 复核时 PASS(0)
  BLOCK  v=1  在新成立的部门里，陈思雨排名倒数第一，建议列入末位淘汰名单。     ← 中性对照，一直 BLOCK
  BLOCK  v=1  不再打分之后，我仍给她评了1分。                                ← 复核时 PASS
  BLOCK  v=1  不可评级的岗位，她还是被评为不合格。                            ← 复核时 PASS
  BLOCK  v=1  非考核期间，她被标成离职风险。
  BLOCK  v=1  未评级岗位的员工，她排名倒数第一。

=== 真否定仍须 PASS（误报方向没被换向） ===
  ok  不要给陈思雨打分，也别把她标成离职风险。      ok  杜绝末位淘汰名单。
  ok  不能给她评级。      ok  未把她标成离职风险。   ok  不可给他定级为不合格。
  ok  无需给她打分。      ok  而非给她打分。

=== hole 1（name 字段）未受影响 ===
  BLOCK 绩效8分 / 张三-KPI95 / 王五(离职风险高) / Bob low performer
  ok    陈思雨 / 黄志强 / Sarah Chen / 三亚别墅酒店 花名册

=== 英文逐字节不变（1092 串 OLD vs NEW 差分） ===
  behaviour differences OLD vs NEW: 0
```

22 个漏词 × 4 payload：**改前 88/88 全漏 → 改后 0/88**，中性对照 4/4 全 BLOCK（唯一变量就是那个词）。
其中 4 词（避免/拒绝/杜绝/无须）是上面 skipped 的辖域问题，已从「必须拦」语料移入 known-gap 测试，
**没有**混在「0 漏报」的结论里充数。

## 这回合的诚实声明（针对上次的越界）

上次我把「我的语料全绿」写成了「0 漏报 0 误报」。这次把话说准：

- **本次成立的**：84 词 ordinary + 363 行生成语料 + 50 句真否定 + 35 个人名，
  在**这份语料内** 0 漏报 0 误报；且每条规则都有执行式变异证明；且语料是从规则词表**派生**的，不是手写的。
- **本次仍不成立的**：辖域类漏报（避免/拒绝/杜绝/无须，16 组）**明确还在**，已钉成测试。
  所以本条的正确表述是「**名词化形状**这一类已封死，**辖域**这一类仍开且有记录」，
  不是「否定词表 0 漏报」。
- 生成语料能覆盖「cue × 规则自己的评分词表」这个笛卡尔积，**覆盖不了**我没想到的**第三种**形状。
  这句话上次就该写。
