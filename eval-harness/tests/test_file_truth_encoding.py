...
def test_gbk_scoring_sheet_never_puts_a_score_on_a_person_over_real_http(client):
    """端到端：给人打分的表，**不管走哪个抽取器**，都不许有任何评分落到人身上。

    🔴 这条测试原本断言的是 `status_code == 422`，那个前提是错的，集成时才发现——
    而且它错的方式很有教育意义，留在这里：

    `service/app.py` 在导入时 `load_dotenv(HERE/".env")`，本机那份 .env 里有真的
    MINIMAX_API_KEY / DASHSCOPE_API_KEY。于是 `/ingest` 走的是 **LLMExtractor**，
    而不是单元测试里那个 HeuristicExtractor。两者对这张表的处理不同：

      · 启发式：把第 4 列「绩效评分」按位置塞进 PersonEntity.tenure（'2分'）
        → validate_extraction 命中 person-score-text → ingest_paths ok=False → 422
      · LLM：压根不产出这个字段 → 没有违规可拦 → 200

    所以 200 **不是红线漏洞**——评分数据一个字节都没落到人身上（见下面的断言）。
    它是「同一份文档被两个入口判成不同结果」。红线的实质保证（人身上不许有评分）
    两条路径都守住了；差的是"要不要连文档一起拒收"这个策略在两条路径上不一致。

    这条测试现在断言**实质**（人身上没有评分），而不是断言一个只在启发式路径成立的状态码。
    422 那条路径由同文件的 test_gbk_scoring_sheet_still_hits_the_red_line 在库层守着。

    UTF-8 版同样是 200（我实测过），所以这跟编码无关——写成 GBK 只是为了和上面那组保持同源。
    """
    resp = client.post("/ingest", files={
    ...
    for person in people:
        leaked_keys = SCORE_KEYS & set(person)
        assert not leaked_keys, f"人对象带上了评分键 {leaked_keys}: {person}"
        # 值也要查：分数可能伪装成别的字段（启发式就把它塞进过 tenure）。
        for key, value in person.items():
            if not isinstance(value, str):
                continue
            assert not re.search(r"\d\s*分|\bscore\b|\brating\b", value), \
                f"人对象的 {key} 里混进了评分值 {value!r}: {person}"
