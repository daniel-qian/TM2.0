...
const SEED_DOCS = [
  ...
  },
  {
    ...
    text: [
      ...
      '进度：58%', '风险：高/雨季场地档期紧张',
      // rich-align-0722/02：里程碑块（3 态），stick 7 断言圆点串。列表在 `阻塞：` 标签处止（不吞）。
      '里程碑：', '- 需求评审（已完成）', '- 登录联调（进行中）', '- 验收交付（未开始）',
      '阻塞：验收口径未确认，法务尚未回复。', '',
      '本周完成登录页联调，下周进入验收。', '',
    ].join('\n'),
  },
  // rich-align-0722/08：SOP 方法库文档（doc_kind 嗅探=unknown → 只投 playbooks + materials，
  // 不加人/项目，不动既有几何行）。5 个 `## 方法：` 小节 → playbooks 屏 2 列 5 卡（stick 12）；
  // `## 说明` 免责段不成卡。适用行=description、标签行=tags。
  {
    name: 'w33-sop.md',
    text: [
      '# 望江咨询 · 管理方法与升级红线', '',
      '## 方法：重大交付跨部门协作闭环',
      '适用：需要多部门配合的重大客户交付。',
      '要点：', '- 交付牵头先经专题协调会对齐资源。', '- 事后组织复盘回填下一次。',
      '标签：交付、跨部门、协调、复盘', '',
      '## 方法：旺季产能协调',
      '适用：多个项目叠加的高峰周。',
      '标签：旺季、产能、排班、协调', '',
      ...
      '标签：项目、周例会、预警、过程管控', '',
      '## 方法：升级与红线判定',
      '适用：超出现场处置范围的合同、安全、舆情情形。',
      '标签：升级、红线、法务、安全', '',
      '## 方法：新人爬坡期公平判断',
      '适用：新人在高峰期的适应与协助。',
      '标签：新人、爬坡、协助、公平', '',
      '## 说明',
      '本方法库为示例演示语料，制度均为原创虚构。', '',
    ].join('\n'),
  },
]
