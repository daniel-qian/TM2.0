...
const ROSTER = [
  ...
  { zone: 'A', name: 'team-map',             cmd: ['eval-harness/tools/verify-team-map.mjs'],                 host: 'preview', backend: true,  dist: false, note: '🔴 上传型门（#106 B3 团队地图 `/map`；真上传 demo-seed 九份中文 md＝每跑一遍造 1 个 context）；**绝不能排在 C 区之后**。守四件会无声坏掉的事：画布真挂上且真拖得动（rzpp 是外部依赖，升一版镜头就可能停在 initialScale 0.5——B1 实测过一次「所有计数判据全绿、只有截图上一片空白露馅」）· 分区/人/项目/连线都对得上花名册（另写一份 derive 就漂，ADR-0034）· focus 点亮与三条复位路（Esc / 回到全景 / 点空白）· 🔴 人身零数字。B3 又添了 HUD 三触发器（搜索 / 部门 chips / 警报药丸）与镜头跟随。⚠ **期望值一律在门这一侧手算**，算 `store.team`（花名册本身，位于被测 derive 的上游）——拿 `deriveGroupFacets` 去验 `buildMapLayout` 是把尺子长在被量的东西上。⚠ 两把尺量同一条红线：文本尺扫人节点子树、**几何尺**扫「带数字的文字矩形压没压在人节点框上」（后者才看得见「数字由另一层画在他头上」，变异 M12 实证 E1 绿而 E2 红）。⚠ 末段整轮切 `lang=en` 再跑一遍红线 + 宽度裁剪（门语料天然全中文＝盲点的反面）。变异实证 21 发见 `.issues/team-map-revival-0804/mutants-b3.mjs` 与 receipt-106-b3.md；其中 M05（rzpp `onInit` 那把锁）**如实存活**——它的独有射程在浏览器里造不出来，A4「换板重新落位」守的是另一把锁。🔴 **后端要第四件 `AVERY_ALLOW_PERSON_SCORING=1`**：⓪ 自证那条要「有人报了自述负载」，关着这个开关时后端根本不投影 self_report，这道门会以「语料不够格」的形态假红（0818 查实；同一条前提也是像素基线那两套的，见 visual-baseline 那行）。⚠ D1b「mini 卡对准它挂着的人」是 0818 补的：入场动画 `both` 填充的 to 帧把居中用的 translateX(-50%) 永久抹掉，**动效开着的世界里卡片偏 98px**，而歪的那一帧当时已经被冻进像素基线当正确答案（reducedMotion 那会儿没生效）。这条必须等 200ms 动画跑完再量——动画进行中它恰好还在正确位置上，早量一帧就是假绿。' },
  ...
  { zone: 'B', name: 'visual-baseline',       cmd: ['node_modules/playwright/cli.js', 'test', '-c', 'eval-harness/visual'], host: 'preview', backend: true, dist: false, note: '像素基线三套（张数别抄这句，自查：`ls eval-harness/visual/__snapshots__/*.png | wc -l`，现 66；⚠ 别数整个目录，里面还躺着一个 07-23 留下的 .bak/）：空态 visual.spec（9屏×2皮×2视口）+ 数据态 visual-data.spec（#68 真上传 demo-seed+gap 种子后采 home/team/projects/room，含 2 张手机差距块专拍）+ /map visual-map.spec（#106 B3，空态/数据态/focus 三态×2皮×2视口）+ /map 收拢态 visual-map-collapsed.spec（#107 B4，收拢/展开×2皮×2视口；**不上传也不吃后端**——80 人合成 fixture 在 Node 侧过真 derive 直接灌 __lite2Store，因为 40 人门槛用 demo-seed 那 16 人永远够不着）；🔴 数据态那两套要后端带 AVERY_DEMO_SEED_DIR **以及 AVERY_ALLOW_PERSON_SCORING=1**（0818 实收：第四件以前没写在任何地方，只活在冻基线那次的 shell 里——关着它时后端不把 self_report 投影到人卡上，地图的组级读数与团队屏的自述行整行消失，于是同一批基线里 map 那 12 张是「开着」冻的、visual-data 那 18 张是「关着」冻的，两套自相矛盾却谁都不红。两份 spec 现在各有一条自证判据盯着它）；🔴 序错（dist 被调包）时这道必假红；重冻要 --update-snapshots 且只在人审对照板通过后，并且冻在**主检出**（__snapshots__/ 是 gitignore 的单机产物，每 worktree 一份，worktree 里冻＝白冻）。⚠ 同一条命令还会跑 visual-determinism.spec.mjs——它不采样，只钉「配置里那几个确定性开关真的落到了浏览器上」：reducedMotion 顶层写法在 playwright 1.61.1 里根本不转发（0818 查实，66 张基线此前全是动效开着冻的），而配置头注释那句现在时的断言两个月没红过一次。它连带扫「有没有别的 spec 自己 newContext 绕开这份配置」。born-red 台架：`node .issues/visual-determinism-0818/mutants-determinism.mjs`（9 发）' },
