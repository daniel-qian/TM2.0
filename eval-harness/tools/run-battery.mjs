...
const ROSTER = [
  ...
  { zone: 'B', name: 'visual-baseline',       cmd: ['node_modules/playwright/cli.js', 'test', '-c', 'eval-harness/visual'], host: 'preview', backend: true, dist: false, note: '像素基线三套（张数别抄这句，自查：`ls eval-harness/visual/__snapshots__ | wc -l`，现 66）：空态 visual.spec（9屏×2皮×2视口）+ 数据态 visual-data.spec（#68 真上传 demo-seed+gap 种子后采 home/team/projects/room，含 2 张手机差距块专拍）+ /map visual-map.spec（#106 B3，空态/数据态/focus 三态×2皮×2视口）；🔴 数据态那两套要后端带 AVERY_DEMO_SEED_DIR；🔴 序错（dist 被调包）时这道必假红；重冻要 --update-snapshots 且只在人审对照板通过后，并且冻在**主检出**（__snapshots__/ 是 gitignore 的单机产物，每 worktree 一份，worktree 里冻＝白冻）。⚠ 同一条命令还会跑 visual-determinism.spec.mjs——它不采样，只钉「配置里那几个确定性开关真的落到了浏览器上」：reducedMotion 顶层写法在 playwright 1.61.1 里根本不转发（0818 查实，66 张基线此前全是动效开着冻的），而配置头注释那句现在时的断言两个月没红过一次。它连带扫「有没有别的 spec 自己 newContext 绕开这份配置」。born-red 台架：`node .issues/visual-determinism-0818/mutants-determinism.mjs`（9 发）' },
