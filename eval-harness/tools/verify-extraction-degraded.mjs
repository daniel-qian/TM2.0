...
//
// ## 判据（每条都配对照，防空真）
//   ⓪ 自证：改包真的生效了（拦到了那一发，且响应体里确实带着我们注入的值）。
//      少了它，②③ 全是「横幅没出现是因为请求压根没被拦到」的假红/假绿温床。
//   ① 基线：**注入之前**屏上没有横幅（②的对照基准；没有它，②等于 0===0 的空真）。
//   ② degraded 从**网络层**进来 → 横幅在屏上，且三句话都在（标题/正文/重试指引）。
//   ③ 🔴 **刷新之后横幅还在**。这是本票的中心句：`extraction_mode` 只有两个**写口**发，
//      `GET /team/{id}` 是读口、不重跑抽取——只活在内存里的警告会在刷新的一瞬间消失，
//      而 0811 那位用户的真实动线正是「传完 → 看了会儿 → 翻页/刷新」。
...
const WEEKLY = ['# 别墅套餐推广', '负责人：周雅婷', '状态：进行中', '截止：2026-09-30'].join('\n')
...
await page.route(/\/(ingest|team\/[^/]+\/files)$/, async (route) => {
  ...
  if (req.method() !== 'POST' || inject === null) return route.continue()
  ...
  body.extraction_mode = inject
  patched.push({ url: req.url(), mode: inject })
...
inject = 'heuristic'
await upload([{ name: '周报-h.md', text: WEEKLY }], 'appendFiles')
...
// ── ⑤ 'llm'：正常态同样不出 ──────────────────────────────────────────────────────────────
inject = 'llm'
await upload([{ name: '周报-l.md', text: WEEKLY }], 'appendFiles')
...
inject = 'degraded'
await upload([{ name: '周报-d.md', text: WEEKLY }], 'appendFiles')
...
inject = 'llm'
await upload([{ name: '周报-good.md', text: WEEKLY }], 'appendFiles')
...
inject = 'degraded'
await upload([{ name: '周报-d2.md', text: WEEKLY }], 'appendFiles')
