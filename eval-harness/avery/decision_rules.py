...
# T9（gap2 #58）· 本期表单还差人没交：链接到期前这么多小时开始上今天页。
# 48 小时怎么来的：链接是 7 天期（`form.EXPIRY_DAYS`），提早太多就是每周有五天在催人，
# 提早太少（比如 6 小时）则经理看到时已经来不及转发一次提醒。剩两天 = 还追得回来的最后窗口。
# 用**小时**而不是天：判据比的是 `expires_at` 这个带时区的**瞬间**（表单是按刻建行的，同一天
# 铸的两批链接可以差十几个小时），折成天粒度会让「还剩 47 小时」和「还剩 1 小时」变成同一件事。
FORM_DUE_SOON_HOURS = 48
...
RULES: tuple[Rule, ...] = (
    ...
    Rule("R-UNCLASSIFIED", NEEDS_CONFIRMATION,
         ...
         "（未命中任何规则）"),
    # gap2 T9（#58）· 本期常驻表单还差人没交，而链接快到期了。
    # 🔴 这是规则表里**第一条不是关于项目的规则**——它的主体是「这一期的这张表」，不是某张项目卡
    # （`decision_grading.grade_form_period` 造它自己的主体，`grade_project` 永远命不中它，
    # 那里的匹配器读的 `subject.forms` 在项目主体上恒为 None）。为什么不挂到项目上：表单是**按人**
    # 铸的，绝大多数链接根本没绑项目；挂上去等于在每张项目卡上重复印同一句话，还漏掉没有项目的公司。
    # 🔴 措辞（ADR-0018）：只说「还差 N 人没交」这个可核对的事实，不说「谁不配合」「谁拖延」——
    # 没交的原因我们一无所知（人可能在休假、可能压根没收到转发）。判断留给经理。
    # ⚠ 这条的 `title_zh` 里**不写那个人数**：它每次命中都不同（见 RULE_DYNAMIC_PARAMS）。
    # 别的规则的 title_zh 用 f-string 把阈值插进来是因为那些是静态配置，写死在说明书里为真；
    # 这里写死一个数就是在客户口径说明书上印一句只在某一次为真的话。
    Rule("R-FORM-MISSING", NEEDS_CONFIRMATION,
         f"本期还有人没交，而最早的一条链接 {FORM_DUE_SOON_HOURS} 小时内就要到期",
         "本期表单链接的状态 + 到期时间"),
...
RULE_PARAMS: dict[str, dict[str, int]] = {
    ...
    "R-FORM-MISSING": {"hours": FORM_DUE_SOON_HOURS},
}

# --- 每次命中都不同的占位符（gap2 T9 · #58）-----------------------------------------------------
#
# 上面那张表的红线原地不动：`RULE_PARAMS` 只放**静态阈值**。这张表登记的是另一种东西——
# **参数名**，值由匹配器在命中的那一刻算出来，随 `RuleHit.params` 逐条发（见
# `decision_grading.RuleHit.to_dict`）。
#
# 🔴 为什么当初禁掉「每次命中都不同的数」、而这条又开了口子——两个理由都要对得上，否则就是在
# 拆自己的碑：
#   ① 当初被删掉的是「已过 62 天」那一类。它的毛病不是"动态"，是**算得不准**：实际天数要拿服务端
#      本地 date 减一个带时区的 UTC 时间戳，跨日就差一天，写进句子就是屏幕上一句假话。而且它
#      **已经有**一个语言中立又逐字为真的载体——evidence 里那行 `uploaded_at="2026-06-20"`，
#      经理自己会看日历。既然有更好的载体，那个会算错的数就没有存在理由。
#   ② 「还差 N 人没交」两条都不成立：N 是我们自己库里那一期 open 行的**条数**，是精确整数，没有
#      单位换算、没有跨日边界；而且它**没有**替代载体——句子里不写这个数，就只剩「本期还有人
#      没交」，经理无法据此决定要不要现在就去催（差 1 个和差 9 个是两件事）。
#
# 🔴 而且它不可能与证据面对不上：`_m_form_missing` 里这个 N **就是** evidence 那一列的长度
#    （`params={"n": len(lines)}`），不是另算一遍的第二个数。句子说差 3 人，底下就正好摆着 3 行
#    「谁」。一个会漂的第二事实源在这里结构上就不存在。
RULE_DYNAMIC_PARAMS: dict[str, tuple[str, ...]] = {
    "R-FORM-MISSING": ("n",),
}
