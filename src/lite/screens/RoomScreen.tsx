...
// function of `t` (not a module-level const) since it now depends on locale.
  ...
  return (
    <section className="nexus-terminal" aria-label={t.lite.terminalAria}>
      <header className="nexus-terminal-bar" aria-hidden="true">
        ...
        <span className="nexus-terminal-title">{t.lite.terminalTitle}</span>
      </header>
      <div className="nexus-terminal-log" ref={logRef}>
        {lines.map((line) => {
          const lineMeta = meta[line.speaker]
          const prefix = line.type === 'manifest' ? t.lite.roomManifestLabel : lineMeta.label
