...
export interface LiteModalProps {
  ...
  ariaLabel: string
  /** 背景关闭按钮的无障碍名（背景是个真 button，键盘用户也能关）。 */
...
export function LiteModal({
  ...
  // 🔴 弹层的"可见"不许押在动画帧上。后台标签页里 requestAnimationFrame 不跑，framer-motion
  // 的帧循环随之停摆——实测（feat-052，document.visibilityState='hidden'）层与面板会一直卡在
  // initial 值上，opacity 恒为 0，弹层等于没出现。首访向导是自动弹的：浏览器恢复会话时在后台
  // 标签页开着 Avery，正是这个场景。所以文档不可见时直接落终态（initial={false} = 不放进场
  // 动画、一上来就是最终样子），可见时才走动画。正常路径零影响。
  const skipEnter = typeof document !== 'undefined' && document.visibilityState === 'hidden'
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div
          key="lite-modal"
          className={`lite-modal-layer${layerClassName ? ` ${layerClassName}` : ''}`}
        ...
        >
          ...
          <motion.div
            ref={panelRef}
            className={`lite-modal-panel${panelClassName ? ` ${panelClassName}` : ''}`}
            role="dialog"
            aria-modal="true"
