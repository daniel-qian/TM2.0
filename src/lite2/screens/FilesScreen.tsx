...
import {
  ChangesZoneIcon, FilesZoneIcon, FormsZoneIcon, SearchIcon, TrashIcon, UploadIcon,
} from '../icons'
import {
...
// #88 · 只剩三个真分区（`changes` 是 #85 同日加的「资料更新」流水）。
// `empty`（清空这份档案）**不是分区**——它是销毁类动作，点了弹硬确认、不换工作台
// （`data-files-zone="empty"` 那一行单独渲染，不进这个联合类型）。
type FilesZoneId = 'files' | 'changes' | 'forms'
...
export function FilesScreen() {
  ...
  const zoneOn: Record<FilesZoneId, boolean> = {
    files: true, changes: changesZoneOn, forms: formsZoneOn,
  }
