...
export const zh: Dict = {
  ...
  "lite2": {
    ...
    // ── B3 · HUD-lite ──
    // 🔴 地图的搜索框有**自己的**提示语，尽管**匹配**走的仍是顶栏那把 `searchTeam`
    //（同一把尺、同一批结果）。理由：桌面 /map 上两个框同时在画面里，而照抄顶栏文案的话
    // 它们逐字一模一样、干的却是两件事——顶栏打开档案，这个把板上那一撮点亮。
    // 两个一样的标签配两个不同的动作就是那个混淆点；「单一尺子」管的是**一个词怎么算命中**，
    // 不是「这个框自称要干什么」。
    "mapSearchPlaceholder": "在图上找人或项目…",
    "mapSearchAria": "在图上找人或项目",
    // searchEmpty（零命中）照旧复用：「没找到」这句话在哪儿说都是同一句。
    // 它刻意**不带数字**：「41 条里的 8 条」会让人把这列表读成一份排名，而该做的事两种写法
    // 都一样——再多打几个字。
    "mapSearchMore": "还有更多——再多打几个字缩小范围。",
    "mapZonesAria": "部门",
    // 🔴 chip 上**只有部门名**。deriveGroupFacets 现成就给了人数，这里刻意丢掉：
    // 一排「后厨 4 · 前厅 2 · 客房 1」就是一张部门人数排行榜（ADR-0023），
    // 而这一页有「人身零数字」的红线。
