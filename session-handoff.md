...
- **★ AFK 串行链 `feat/live-core-015-018`（07-05，Danny 授权逐步自合并、终验收）**：f965bad docs → 4517f0e feat-015（pytest 141）→ 4e1bac0 feat-016（pytest 169，红线抽取门绿）→ fae493f feat-017 WIP → **feat-017 ✅ done（integrator runtime 验证：story 零回归 + rail seek 1→5/26、live 优雅 + UploadPanel、红线净、zh 是 M3）**。下一步 **feat-018（双端部署）串行**（部署 config+smoke AFK；真部署/promote/域名归 Danny）。主 `main` 不动，Danny 分支终验收后 merge。
