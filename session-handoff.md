...
当日全交付**（8 commit 攒本地 main `0481c96..2be211c` 未推），战役③资料库**盘问定稿出 PRD**。
**✅ Danny 已对 6 张对照截图逐张验收（原话「这些都没有问题」）；push 收尾时未拍板放行**——
下个 session 第一件事：问一句「现在推还是连资料库战役一起推」，然后照答案走。
前后端都有改动（contract/engine/tools/live_input），**push 后要 swap 容器**（07-28 姿势）。

## 本日交付（8 commit，全绿）
1. `0481c96` 酒店经理 persona check 首轮：九条分级发现（P1 问答三重劝退实锤=2145 字符报告卡
   +画板截断+英文罐头）→ `.issues/feedback-0729/persona-review-0729.md`
2. `c98fbb5` **大白话命名 pass（ADR-0031，Danny 逐词审字）**：5 tab（今天/团队/问 Avery/
   值得注意/完整版预告）+ 屏内词族 + 动作词统一「加到待办」+ 3 处「会议室」误译归一；
   反SaaS 第三次松绑（人情味红线 + Nexus/现实差距锁定不动）；v01 冻结壳词表有意分叉；
   门三处联动 + verify-switchers 选择器修复（全局首匹配陷阱）
3. `08c82c3` **画板退役**（输出形态/01）：议事室改 scroll→frame 纵向语法；门相位 H 翻转
4. `c37b777` **砍契约样板**（/02）：REQUIRED 9→3，四块常量文退役（absent≠none 全链），
   卡文本 -43%；v01/v02 coerce 不再编造 medium/none 默认值
5. `618c991` **分流短答**（/03）：answer_direct 出口（同吃 cite 闸）→「Avery 的回答」气泡；
   mock 正则宁漏勿错杀；**电池 25→26 道**（新门 answer-split）
   回执：`.issues/output-form-0729/receipt.md`（含诚实边界）· kickoff 同目录
6. `c7621b0`/`2be211c` handoff ×2 + `d3a503d` **资料库战役 PRD**（盘问 8 项拍板+4 切片+
   4 独立票——含「笔记升级真记忆」T1，事实已查实：现状不回流 recall）

## 环境实况（收尾态）
- 本地 preview/mock **已杀干净**（验收已走完，不留挂机）。重起姿势照 runbook：
  build+preview `--mode development --host 127.0.0.1` + mock 三件套 8137。生产未动（未 push）。
- 验证账实：离线套 3467/0 · 全电池 26/26（本日四轮全量；电池因新门 answer-split 25→26 道）·
  像素基线本机重冻两次（命名全板漂移 + 门谱系确认议事室像素只拍空闲态——记档在 receipt）。
- 验收证据：6 张对照截图（改造前画板报告卡 → 短答气泡 → 判断类判读卡 → 砍样板卡尾 →
  满态首页/团队屏新命名）已发 Danny 逐张过，全通过。

## 🔴 下一步（按优先级）
1. **Danny 验收 → push 上产**（5 commit 一次上；后端有改动（contract/engine/tools/live_input），
   **前后端要同上 + swap 容器**，照 07-28 部署姿势）。⚠ 上产后 partner 侧若有人依赖旧 9 字段
   全量返回，砍样板是行为变化——receipt 已记口径（9 字段=上限形状，按需出现）。
2. **真 brain 分流判据从未真跑**：生产 LLM 链路零调用问题（07-28 agent 线 #4）现在更要紧——
   分流的真 LLM 层（CHAIN_HINT 判据）只有 mock 层被门钉死。上产后手动走一次真 brain 提问取证。
3. **战役③已盘问定稿 → `.issues/files-hub-0729/prd.md`（d3a503d）**：资料库升 tab+砍
   完整版预告、v1 三能力、团队屏零文件元素、笔记升级真记忆（独立票 T1）等 8 项拍板 +
   4 切片 + 4 独立票，**下个 session 照 PRD 开发，勿再重新推导**。④design token 拍板已定
   kickoff 未写；persona 遗留 P2/P3/P4 归后续片。
4. 07-28 handoff 的 UI/agent 两线开放项**仍然有效**（真机零覆盖仍是 UI 线头条），见下段。

---

# ⟳ 2026-07-28 · **partner-docs + UIUX 两批已全部上生产；开放项已按 UI / agent 两线分好**（历史·上一段）

**一句话**：origin/main 积压的 8 提交 + 2 篇回执**前后端同上完毕**，生产实测通。
本段只做一件事：**把已知开放项按「下一步两条并行线」分好**，省得 grill 时重新推导。

## 生产实况（截至 07-28 收工）
- 前端 Vercel = `origin/main`（push 即自动上产）。后端容器 `avery` = **`avery-agent:main-20260728-184431`**（= a7c116d）。
  回滚梯退一级 = `avery-prev-20260728-184431`。预检容器**已清**，5173/8137/8138 **全部空闲**。
- 本轮上线内容：红线不再把 `KPI-001` 这类编号误判成人身评分 · 项目状态词表补「暂停」 ·
  `/paperwork` 页 + 6 个静态资产 + 设置菜单入口 + 上传面板两条链接 · 窄屏顶栏单行化 + 空态卡让位 ·
  两道门的 IPv6 修复 · demo 克隆 GC 与引导免锁**随本次 swap 首次真正生效**。
- 证据：全电池 **24/25**（唯一红=像素基线，11 张漂移逐张归因、无一无法解释，已重冻）·
  离线套 **3464/0** · 两条探针 exit 0 · 上线后 13-agent 六面验收五面 PASS / demo-e2e PARTIAL。
  详见 `.issues/partner-docs-0728/receipt-deploy-0728.md`（含完整发现台账）。

## 🔴 不归工程线：法律件三份的对外风险
公开可下的 DPA 草案正文带我方谈判底牌、隐私件称数据存境内而后端在法兰克福。
**Danny 已定：归合伙人负责，工程线不处理。** 记录在回执里，别再当 bug 捡起来。

---

## 交给 **UI / UIUX 线** 的开放项

1. **真机零覆盖**——所有几何都是桌面 Chromium 改视口量的。**iOS Safari / 微信内置浏览器一次没跑过**，
   而合伙人上一轮那两个 bug 正是真机逮到的。sticky 顶栏 + 100vh 在 iOS 上历来另一套行为。**优先级最高。**
2. **满态布局在生产零覆盖**：进满态要上传或领 demo，只读铁律禁 → 本地满态绿 ≠ 生产满态绿
   （同类历史教训见 [[offline-suite-blind-to-pg-persistence]]）。
3. **en 的退化边界已经贴脸**：1280×720 en / 900×700 en 的操作手册空态卡 `margin-block` 实测 **0**
   （卡顶死在让位线 96px 上），1280×720 en 已在卡内滚、底部 CTA 下沿被裁。**en 文案再长或视口再矮 20px 就露馅。**
4. **860 断点外侧余量薄**：884(zh)/954(en) 的 notes 屏「首内容距带底」= 221.8 / 204.6px，
   离门的 240 上限只剩 18–35px。不是 bug，但判据在采样范围外几乎没余量。
5. **断点动物园**：lite2.css 一个文件八个断点（560/620/720/860/861/880/1100/1366），00-base 只有 860。纯重构，单开一场。
6. **像素基线 tracked 与否未拍板**：`.gitignore:34` 排除它，而 `playwright.config.mjs` 注释写「基线 PNG 提交在 __snapshots__/」——
   注释与实际相反。后果：像素门是单机产物，换机器/换 agent 没基线可比，进不了 CI。
7. 上传面板两条链接 **href 完全相同**（无锚点），点「你上传的内容怎么处理」落地时那份文件在折叠线外且默认收起。
8. `.lite-onboard-card` 是**生产上的死选择器**（闸门在场它也恒 0）。谁照抄它写门＝永远绿的假门。判闸门只能用 `.lite-onboard` 或 `[data-gate-doors]`。
9. i18n 注释与实现相反：en.ts/zh.ts 写「设置菜单第四行，追加在最后一行之后」，实测 DOM 索引 i=2（第 3 行）。

## 交给 **agent / 抽取线** 的开放项

1. **「暂停」这修在生产的实际收益从未端到端量过**：生产是 **llm 抽取模式**，`_norm_status` 只在模型输出
   落词表外时才兜底（`llm_extract.py` 提示词直接要求模型只吐 on-track|at-risk|blocked|done）。
   被量过的是 heuristic 兜底路径。**且已入库旧数据不回填。**
2. **我们自己发的表在自己坑自己**：02 表状态下拉含「未开始」，而 `_norm_status('未开始')=''` 是有意为之
   （测试钉死）→ 用户从我们给的下拉里选它，卡上仍显示「状态未提及」。**最容易被合伙人当 bug 报回来。**
3. **红线改「丢字段而非整批拒绝」那条线在飞**（改 `pipeline.py` 执行层），不在本次 8 提交里。
   合入后后端要重跑电池 + 重新 swap；届时 `probe-redline-07.py` 顶部描述会过时（文件自己写了这个提醒）。
4. **生产 LLM 链路从未被走通过一次**：`llm_calls_remaining` 跨 30 分钟多次采样恒为 2000、进程未重启
   → 该后端进程自启动至今**零次 LLM 调用**。`brain=minimax` / `live=true` 只是**配置回显**
   （`brain_factory.py`: live = env != "mock"），不是存活探测。议事室提问这条路上线后没人验过。
5. **`/health` 无 version/commit 字段** → 「生产跑的就是这个镜像」只由 swap 日志自证、外部不可核。
   加一行就能把这类断言从自证变成可验。

## 环境提醒（两条线都会撞）
- 跑门电池前先读 [[avery-frontend-gate-setup]]：build+preview（**不用 vite dev**，它返 200 但 app 不 boot）、
  `--mode development` 不能省、preview 加 `--host 127.0.0.1`、像素基线是单机产物。
- 跑 pytest **必须**带四个 deselect（本机 `.env` 有真 key）。
- Windows 控制台是 GBK：跑带中文输出的 py 脚本要 `PYTHONIOENCODING=utf-8`，否则崩在 print 而不是逻辑。
