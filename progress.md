...
**Last Updated:** 2026-08-08（演习批三票落 main 且主检出独立复核绿（3974/0）→ **复演开场十分钟
又逮三条** → 开 #65/#66/#67、卡片已发（#66+#67 捆一个 session）；仍未 push、未上产）
...
1. **演习第 2 轮批（#65 / #66+#67）先做完**（0808 复演开场十分钟的三条发现，kickoff 卡片已发，
   两个 AFK session：#65 独立；**#66+#67 捆一个 session**——都动 AskRefComposer/议事室一带，
   拆开并行必撞车）：
   **#65** 今天页差距块默认展开（保留收起钮，Danny 拍板）·
   **#66** @ 候选弹层被遮挡/错位（三层病根已侦察钉死进票面：恒上弹零碰撞检测 /
   议事室运行态被 `.scene` overflow:hidden 硬裁 / **空态包含块误落 `.nexus-empty`**）·
   **#67** 预填入口全量引用化（8 入口清单在票面；零签名改动，#64 已打通通路——
   「项目没算进去」是误会，@ 弹层四类齐全，缺的是入口层没接）。
   ⚠ 两 session 合 main 串行、后合的在主检出重冻像素（#65 今天页必漂）并复跑电池。
2. **复演（续）**（第 2 轮批做完后，本地）：环境随起随用——`preview_start rehearsal-api /
   rehearsal-web`（launch.json 两条配置在，本地未提交；后端脚本 `.issues/gap2-0807/rehearsal-api.ps1`）。
   库 `rehearsal0808` 数据保留；脑真 MiniMax（问答几分钱级），抽取/检索离线。复演时顺手验：
   ① 演习花名册转标准 markdown 表格再传（#61 触发动作）；
   ② 差距对照卡进今天页后的动线 + 默认展开（#63/#65 真验收）；
   ③ **@ 引用打真脑**（#64 真验收——离线 spy-brain 验的是「注入到达 conversation」，
   「注入被用上」只有真脑能看）；④ 从人卡/项目卡点「去问 Avery」应带 chip（#67 真验收）。
   第 1 轮已走过：示例团队 → 铸链 → 员工填 → 快问（真脑）。
3. **统一上产**（连同 gap2 三票 + 两轮演习批一起）。
