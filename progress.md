...
1. **#72 建议追问 chips + 快问触发收敛**（wave 2，**卡片已发**——依赖 #71 已落 main，
   wave 1 主检出独立复核绿：pytest 3996/0 与两票新增用例数严丝合缝、init.sh 绿、
   50 张像素基线 mtime 未动=真比对）。卡片提示里已点名 `receipt-69-71` 的
   「刻意没做」两条（尾轮/历史轮分工、`ask` 卡随新一轮退场值得重裁）。
