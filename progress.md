...
**Last Updated:** 2026-08-07（T8 收官自检 → 统一上产 → HITL 端到端轮 → **HITL 三条修复全部补完并上产**；
差距战役 T1–T8 整条链已在生产上真跑过、真验过）
...
1. **T8 留的两条记录还在桌上**（只记录未改，等拍板）：
   ① 议事室引用编号是 `facts.md:<行号>` 而非客户自己的文档名（人卡/今天页已是 ADR-0028 形状）；
   ② 今天页证据行是 `字段="值"` 的机器形状（ADR-0033 明定），演示时建议口头带一句怎么核对。
2. **三亚 demo 脚本更新**（碰头在下周）：新能力进 demo 主线；快问改为现场新建（旧 ask 死链不自愈）。
   `t8-shots/` 那 20 张可直接用。
3. **给 `/health` 加一行版本字段**（镜像 tag 或 commit）：现在「生产跑的是哪个 commit」外部不可核，
