...
14 提交全上：前端 Vercel 自动部署 ● Ready；后端 swap 到 `avery-agent:main-20260730-125353`
（SWAP SUCCESS，健康闸 1×2s；回滚梯 `avery-prev-20260730-125353` 保留）。

**🔴 那个「未验」终于验了，而且是 born-red 实锤的。** 07-28 的回执立过一条取舍：预检容器与
生产容器连同一个库，所以后端修复只在离线层验。本轮的修复恰恰是**写路径**，离线层
（memory registry）结构上复现不出来——照那条取舍走就等于永远验不到。绕法：拿
`/demo/claim` 的 **ephemeral 克隆**当被试（不是客户真 context，但走同一个生产 Postgres、
同一条 `get→改→put`）。同一条探针三跑：

| 跑 | 镜像 | 加项目之后再下载 idx 0 |
| --- | --- | --- |
| 8138 预检 · 新镜像 | 新 | **200，sha 与写入前相同** |
| 8137 生产 · **旧镜像** | 旧 | **404** ← born-red，bug 当时正 live |
| 8137 生产 · swap 后 | 新 | **200，sha 相同** |

没有第二跑，前两跑的绿只能证明探针跑通、证明不了探针能红。

**顺带量到一条真实的体验/成本问题（不在本战役范围，没动，值得单开票）**：一次手编写入要
**40–45 秒**（三跑分别 44.5s / 40.8s / 42.2s）——`put()` 会重嵌整个语料，而手编 CRUD 是
`get→改→put`。用户点一下「添加项目」要等 40 秒，且每次都在烧 DashScope 嵌入调用。
重嵌应当只在语料真的变了时发生。

详见 `.issues/files-hub-0729/receipt-deploy-0730.md`。
