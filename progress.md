...
**Last Updated:** 2026-08-08（gap2 三票合 main 后主检出独立复核绿 → 本地演习第 1 轮跑一半暂停：
四条真发现 → 新开 **#63/#64** + 认领 **#61**，三票做完再复演；仍未 push、未上产）
...
1. **演习批三票（#64/#61/#63）先做完**（0808 演习第 1 轮的产出，kickoff 卡片已发，每票一个 AFK session）：
   **#64** 议事室 @ 引用回归（拍板：**一步到位结构化**——refs 进契约、被引实体读数保证进上下文；
   考古：@ 引用是 story 壳 TeamComposer 的，lite2 替身 LiteComposer 在 #47/`7b03982` 退役成
   悬浮胶囊时没搬过去，功能自 07-22 丢失）·
   **#61** 花名册解析吃下标准 markdown 表格（外框竖线静默吞表）·
   **#63** 「值得注意」并进「今天」退 tab（浅合；演习中 Danny 连自己都分不清两个「对不上」界面）。
   ⚠ #63 与 #64 都动今天页/议事室一带——合 main 串行、后合的在主检出重冻像素。
2. **复演**（三票做完后，本地）：环境随起随用——`preview_start rehearsal-api / rehearsal-web`
   ...
   替身答案把快问体验做失真了），抽取/检索仍离线免费。第 1 轮已验：上传→表单区→铸链→员工填→
   快问全链在本地能走；demo 克隆上补资料入口按设计隐藏。
3. **统一上产**（票面：落 main 即止，上产由主 session 安排；此时连同演习批一起上）。
