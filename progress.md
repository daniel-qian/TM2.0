...
1. 🔴 **在主检出重冻像素并人眼过（三票唯一的欠账，归 T10）**。理由不是流程洁癖：
   `__snapshots__/` 是 gitignore 的单机产物、**每 worktree 一份**，在 T10 的 worktree 里重冻
   等于没重冻。T10 改了资料库屏（多一段「给这家公司补资料」）与详情浮层（多一个出处角标）。
   ⚠ 像素 spec 跑在 `?transport=stub` 且从不上传 → `contextId` 恒 null → **补资料那一段整段不渲染**
   （三条否决的第一条就是没有 contextId），常驻表单区同理（T9/T11 那两条注释说的是同一件事）。
   所以真正会动的是第②段那两句改口的文案 + 标题「上传新一批」→「另建一份画像」。
   **逐张人眼过对照板，再决定重冻。**
2. **统一上产**（票面：落 main 即止，上产由主 session 安排）。
...
3. **Danny 用户视角端到端演习**（三票上产之后）：从零自己开一个新工作区走全程，
