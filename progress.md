...
1. **统一上产**（gap2 三票 + 三轮演习批 + #68 + #70 + #69/#71 + #72）。🔴 push 与换后端容器
   同窗口；**0015 必须落地**；上产后先设 `AVERY_PUBLIC_BASE` 再验表单。
2. **复演（第 4 轮）**：`preview_start rehearsal-api / rehearsal-web`；库 `rehearsal0808` 保留；
   脑真 MiniMax。验：①预填=灰色提示不占正文、空着发不出去 ②打 @ 零筛选文件候选可引原话
   ③问两问不覆盖、追问接得住指代 ④**回答下方建议追问可点（#72 已落）——重点看真 brain
   会不会真往 followup_questions 槽里填、填得像不像话**（离线 mock 只证明管道通）
   ⑤快问卡不再"问啥都弹"（提人名的事实查询不弹、判断类照弹）。
3. **#74 file-ref-id**（#70 核实出的真缺陷，纯前端契约票）。后端一行不用动。
4. **T8 两条记录**：① 议事室引用编号形状；② 今天页证据行机器形状（ADR-0033）。
5. **给 `/health` 加版本字段**。
6. carry-over：**持久对话线程（0808 拍板不做）** · **议事室流卡死 composer 锁死无前端超时
   （#71 已知边界）** · 判读卡 4 段死渲染 + 后端已发前端未消费 7 类字段 · r2 未开票发现 ·
   gate-run 迁移 · files-hub #26–#29 · 换血抢救 #31/#32 · v01 退役 #33 · 真机零覆盖
   （iOS/微信，最高优）· 成本票 #30 · 真 brain 分流取证 · 全量 feat-063 ·
   **#73 composer 现场上传**（后排不排期）。
