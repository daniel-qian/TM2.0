...
**Last Updated:** 2026-08-09（**0808 UIUX 重构战役 wave 1 · S1 落地**：#75 议事室 Claude 化
+ #73 现场附件。并行线 S2（#76/#77/#74）另账。仍未 push、未上产）

## Current State

- **git**：`main` = 差距战役八票 + gap2 三票 + 三轮演习批 + #68 + #70 + #69+#71 + #72
  + **#75+#73**（回执 `.issues/redesign-0808/receipt-75-room-claude.md`）。
  `main...origin/main` **ahead 50+**（以 `git status -sb` 为准），**没有 push**（见 Blockers）。
- **✅ 生产仍停在 08-07 白天那一版**（`main-20260807-190332` = main `99d83f7`）。
  gap2 三票 + 三轮演习批 + #68 + #70 + #69/#71 + #72 + #75/#73 **都没有上产**。
- 🔴 **迁移账（上产时按这个来）**：
  - **T9 需要 `0015_form_submissions_auto_key.sql`**（increment-only、`_ensure_schema()` 自动重放）。
  - **#75/#73 不需要迁移**（纯前端 + 复用既有 `POST /team/{id}/files`，零后端改动）。
...
## 本轮做完的（2026-08-09 · #75 议事室 Claude 化 + #73 现场附件）

回执：`.issues/redesign-0808/receipt-75-room-claude.md`（含 8 条变异台账、
开工侦察查出来的 9 条与既有认知不同的事实、刻意留下的账）。
开工前设计裁定：`.issues/redesign-0808/design-75-73.md`。
...
- **停止生成**：`LiveRunState.status` 加第五个值 `'interrupted'`（**不是**加布尔旁挂字段——
  那是 fail-open）。假 complete 的病根是 `streamSource` onDone 的**黑名单**兜底，已改白名单；
  abort 在 source 层包装记账（顺带补上 stubTransport abort 从不 onDone 的另一半缺口）。
...
- **#73 附件**：附件钮 → append 入库 → 等待态 → 完成挂 file ref chip（**用回执的 `source_key`**，
  不是 `store.files[].filename`）；上限**选文件时就预检**（10 个 / 10 MiB / 批 32 MiB），超限零请求。
- **门**：新门 `verify-room-claude-rework` **46 判据**已入册 A 区；at-references（64 判据）、
  room-conversation（42）、snippet 相位 F1/F2/H 全部改判。**A 32/32 · B 3/3 · C 3/3 · pytest 4010/0**。
- **人眼过 26 张**（双视口 × 双皮 + 无材料态），逮到并修了三处门全绿但眼睛看得见的问题：
  chips 居中（story 居中卡的遗留对齐）、手机态 composer 换行崩、HUD 眉标与「已停止」自相矛盾。

## 上几轮做完的（详情全在各自回执，这里只留会影响下一个人的）

- **#72 建议追问 chips + 快问收敛**——回执 `receipt-72-followup-suggestions.md`。
  chips 点击即发；触发判据走**文种感知词边界**；`answer_kind` 语义闸（短答终局不弹卡）；
  撤卡重裁（动过的/已发出的不撤）；`askLive` 加 store 级 busy 闸。
- **#69+#71 会话流+灰提示**——`turns: LiveTurn[]`（`run` 只是尾轮镜像）；后端 `history`
  additive optional；离开议事室/刷新=对话结束（刻意不持久化）；文案长度闸开显示宽度不开字符数。
- **#70 @ 文件引用两修** · **#68 数据态像素基线** · **#66+#67** @ 弹层空间感知 + askRefs 唯一构造器
  · **#65 / #64 / #63 / #61 / T9–T11**。

## What's Next（按优先级）

1. **0808 UIUX 重构战役续跑**（档案 `.issues/redesign-0808/tickets.md`）：
   - **wave 1**：S1=#75+#73 ✅ 本轮落地；**S2=#76 资料库 IA 重排 + #77 删除文件 + #74 file-ref-id**
     （并行线，另账；两线都动 `store.ts` 不同段落，**后合者处理冲突并复跑全电池**）。
...
- **`LiteRoomHistory` 里回放的短答仍是纯文本**，与会话流里的 markdown 渲染不一致（#75 刻意
  不动，#78 地盘）。→ **#78**
- **file ref 的 id 契约没根治**：#75 用回执 `source_key` 走「今天能对的那条路」；
  `store.files[].filename` 撞名不可分辨这件事归 **#74 / S2**。
- **上传上限三处散落**（`src/lite2/uploadLimits.ts` / `UploadPanel.tsx:28` / `guards.py`），
  已互指注释，收敛归 **#76 / S2**。**没有任何端点暴露上限**——前端只能自己维护一份，
  且必须对齐**生产 env**（10 个/10 MiB）而不是 `guards.py` 的默认值（15 个/8 MiB）。
...
- **switchContext 换公司时 `turns`/`run` 不清**（adoptContext 清单没有它们）——若能在议事室
  现场切公司，A 公司的问答会挂在 B 公司名下。#72 已补 ask 四件，turns 那半没动。
...
- 🔴 **变异活下来 ≠ 门有洞**（#75 实收）：M-A 第一版改的是**两态共用**的宽度，
  而判据判的是「前后不变」这个不变量，它正确地保持为真。**先看变异有没有真的碰到被判的那个性质。**
