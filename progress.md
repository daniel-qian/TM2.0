...
✅ **上传管线已在生产上首验（2026-08-19，Danny 拍板跑）**：真上传 → 异步抽取 → 结构化 → RAG 问答
四段全通，**19 条判据 19 条兑现**，回执 `.issues/deploy-0817/receipt-smoke-0819.md`。
实测：存单 **1.34 s** 回来 · 抽取 `processing → done` **44.8 s**（2237 字节中文花名册 / 28 chunks）·
`/advise` **113 s**。抽取 **人 10/10、项目 3/3**、中文姓名逐字对得上；`evidence` 五条全带原文行号
（`facts.md:65/66/96/108/64`），且引到了**只在这份文件里出现的「380 万」**——RAG 真读到上传字节的硬证据。
测试档案已清空（带对照基准 `1/10/3 → 0/0/0`）并标 `ephemeral=true` 交给产品自己的 GC。

⚠️ **冒烟仍没覆盖到的**：`/demo/claim` 没跑（示例团队母本在但没真 claim 过）· 只传了 1 份 txt
（多文件 / PDF / 补传合并 / 红线拒绝都没走）· 没测并发（#102 CAS 未合）· 没走账号路径（用的 owner_token 游客路）。
📌 一条产品观察（不开票）：问「我该找谁」时答案路由到向上借力（陈立文/何秀英）+ 外部经办人，
**没点项目负责人马涛**。讲得通但演示时可能被问到。
