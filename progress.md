...
- **git**：`main` 与 `origin/main` 推平、工作树干净；**最后一个产品 commit = `8769f6b`**（07-30 部署基线，其后 main 上只有 docs/快照类提交）。生产 = main：前端 Vercel 自动部署（averylite.dannyqian.com）；后端容器 `avery-agent:main-20260730-125353`（构建自 `25562d1`，产品代码同 `8769f6b`，见 receipt-deploy-0730），回滚梯 `avery-prev-20260730-125353` 保留。
