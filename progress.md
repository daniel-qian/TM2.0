...
- **后端离线套基线：`TZ=UTC` → 4114 passed · 0 failed · 136 deselected · 4 xfailed**（约 128s）。
  = 上一基线 4076 + #87 的 38 条（`test_entity_lineage_t87.py`，另 3 条 `@needs_db` 进 deselected）。
  ✅ **任何红都是你的。**
- **真库套（@needs_db）**：throwaway `avery_t87_test`（docker `teammaster-postgres-1` / pgvector pg17）
  跑 `test_entity_lineage_t87 + test_registry_contract + test_registry_protocol + test_file_append_t10
...
- 🔴 **迁移账（上产时按这个来）**：
  ...
  - 🔴 **#87 需要 `0009_person_keys_allowlist_richalign.sql` 的**就地修订**（allowlist 加 `'lineage'`，
    `want` 与 `ADD` 两处同改）——**不是新增迁移**，迁移头写着「就地改数组、不许叠新迁移」。
...
2. **#85 可以直接吃 #87 的现成件**（回执 §9.3）：`batch_id` 就是「这一批」的确定性名字，不用再靠
   ...
   要用得自己加一个 additive 投影 key **并同步改那条判据**——它会红，那个红是对的。
3. 🔴 **#87 的两条下游，一条有票一条没票**：
