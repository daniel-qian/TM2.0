...
**Last Updated:** 2026-08-08（**演习第 3 轮完毕、第 2 轮修复全过**；本轮四发现 grill 拍板后
开成 #69–#72 + 后排票 #73，wave 1 卡片已发（#69+#71 捆绑 / #70 单独）；两路读码侦察全文
在 #69–#73 票面与本会话；仍未 push、未上产）
...
1. **演习第 3 轮批（wave 1 进行中）**：0808 grill 四拍板——①预填退灰色提示+空文本置灰（#69）
   ②文件引用可发现性+注入真原文（#70）③议事室会话流+带上下文追问、不持久化（#71）
   ④建议追问 chips+快问触发收敛（#72，**wave 2，依赖 #71**）。#69+#71 捆一 session、
   #70 单独 session（卡片已发）；wave 1 落 main 后补 #72 卡片。#73（composer 现场上传）
   后排不排期，侦察账在票面。
2. **复演（第 4 轮）**（wave 都落后）：`preview_start rehearsal-api / rehearsal-web`；
   库 `rehearsal0808` 保留；脑真 MiniMax。验：①预填=灰色提示不占正文 ②@ 零筛选见文件
   候选、file 引用回答真用上原文 ③问两问不覆盖、追问带上下文 ④回答下方建议追问可点、
   快问不再提人名就弹。
3. **统一上产**（gap2 三票 + 三轮演习批 + #68 门侧）。🔴 push 与换后端容器同窗口；
   **0015 必须落地**；上产后先设 `AVERY_PUBLIC_BASE` 再验表单。
4. **T8 两条记录**：① 议事室引用编号形状；② 今天页证据行机器形状（ADR-0033）。
5. **给 `/health` 加版本字段**。
6. carry-over：**持久对话线程（0808 拍板不做，#71 只做 session 内）** · **判读卡 4 段死渲染 +
   后端已发前端未消费 7 类字段（0808 侦察，做 artifacts 票时接）** · r2 未开票发现 ·
